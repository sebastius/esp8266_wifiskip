...
// const char player[] = "00%3A00%3A00%3A00%3A00%3A00"; // bar sparkshack
...
//    }
  ...
  if (digitalRead(randombutton) == HIGH) { // HIGH because i use Normaly Close instead of NOpen buttons)
    // http://jukebox/Classic/plugins/RandomPlay/mix.html?type=track&player=00%3A00%3A00%3A00%3A00%3A00&addOnly=0
    Serial.println("Randomize");
    newrandomplaylist();
    //delay(1000);
  }
...
bool getPage(const char *p0, const char *p1, const char *p2) {
  ...
  client.println();
  Serial.println("Message sent. Hopefully it arrived!");
